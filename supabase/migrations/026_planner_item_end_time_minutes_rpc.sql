-- Replace end_time time with end_time_minutes integer in create_planner_item
-- and update_planner_item.
--
-- Previously end_time was a PostgreSQL time type, which is bounded to 0–23:59.
-- This meant ends_at was always constructed on the same calendar day as starts_at,
-- making overnight items (e.g. a drive starting at 11 PM and arriving at 12:30 AM)
-- impossible to represent.
--
-- end_time_minutes is total minutes elapsed from midnight of the planner item's
-- owner day. Values 0–1439 produce a same-day ends_at. Values >= 1440 produce
-- an ends_at on a later calendar day.
--
-- Examples:
--   780   = 13:00  (1:00 PM, same day)
--   1439  = 23:59  (11:59 PM, same day)
--   1470  = 24:30  (12:30 AM, next day)
--   1530  = 25:30  (1:30 AM, next day)
--
-- starts_at remains anchored to the owner day via start_time time.
-- The database constraint ends_at >= starts_at continues to hold.
--
-- This is a breaking change to the RPC signature. The previous end_time time
-- parameter is dropped and replaced with end_time_minutes integer.

-- ── create_planner_item ───────────────────────────────────────────────────

drop function if exists public.create_planner_item(uuid, uuid, text, text, time, time, text, text, text, text, text, text, jsonb);
...
begin
  ...
  values (
    ...
    case when end_time_minutes is null then null
...
revoke all on function public.create_planner_item(uuid, uuid, text, text, time, integer, text, text, text, text, text, text, jsonb) from public;
revoke all on function public.create_planner_item(uuid, uuid, text, text, time, integer, text, text, text, text, text, text, jsonb) from anon;
grant execute on function public.create_planner_item(uuid, uuid, text, text, time, integer, text, text, text, text, text, text, jsonb) to authenticated;

-- ── update_planner_item ───────────────────────────────────────────────────

drop function if exists public.update_planner_item(uuid, uuid, text, text, time, time, text, text, text, text, text, text, jsonb);
