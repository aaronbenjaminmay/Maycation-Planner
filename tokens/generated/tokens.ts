...
export const colorDangerSurfaceHover = "rgba(168, 75, 75, 0.3)"; // Danger action hover background fill. CSS composite: color.primitive.red-500 × opacity.primitive.30. Applied on :hover over destructive Button and IconButton. 1.5× multiplier on danger.surface (20% → 30%), consistent with success surface/border ratio. Figma: bind fill color to Color/Red 500, fill opacity to Opacity/Danger/SurfaceHover.
...
export const colorIconOnAccent = "#ffffff"; // Icon fill color for icon buttons rendered on accent-colored surfaces (IconButton --primary, IconButton --complete selected state). White icon on teal — intentionally distinct from color.accent.text (#061312) which is for text characters. Do not use color.accent.text for icon fills on accent backgrounds.
...
export const opacityDangerSurfaceHover = 0.3; // Opacity component for Color/Danger/SurfaceHover (Red 500 × 30%). Figma: bind fill opacity on hover paint. CSS: use rgba(168,75,75,0.30) directly.
...
export const opacityInteractiveHover = 0.08; // Opacity for interactive hover overlay fills (Button, IconButton). Applied as an additional paint layer on the component fill stack, not as element-level opacity. Value matches current rendered CSS: rgba(255,255,255,0.08) on hover background.
export const opacityInteractiveHoverBorder = 0.22; // Opacity for interactive control hover border (Button, IconButton). Border brightens from 10% rest state to 22% on hover. CSS: border-color rgba(255,255,255,0.22) on :hover. Figma: bind stroke color to Color/White, stroke opacity to Color/White/Opacity 22.
export const opacityInteractivePrimaryHover = 0.14; // Opacity for primary button hover brightening overlay. White fill at 14% layered over accent fill — visually equivalent to color-mix(in srgb, accent 86%, white 14%). Figma: bind fill color to Color/White, fill opacity to Color/White/Opacity 14.
